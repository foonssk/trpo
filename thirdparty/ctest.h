...
#define CTEST_IMPL_TEARDOWN_FPNAME(sname)                                     \
  CTEST_IMPL_NAME (sname##_teardown_ptr)
...
#define ASSERT_FAIL() assert_fail (__FILE__, __LINE__)
  ...
  void assert_dbl_compare (const char *cmp, double exp, double real,
                           double tol, const char *caller, int line);
...
#define ANSI_NORMAL "\033[0m"
  ...
  void
  assert_str (const char *cmp, const char *exp, const char *real,
              const char *caller, int line)
  {
    if ((!exp ^ !real)
        || (exp
            && ((cmp[1] == '='
                 && ((cmp[0] == '=') ^ (strcmp (exp, real) == 0)))
                || (cmp[1] == '~'
                    && ((cmp[0] == '=') ^ (strstr (exp, real) != NULL))))))
      {
        CTEST_ERR ("%s:%d  assertion failed, '%s' %s '%s'", caller, line, exp,
                   cmp, real);
      }
  }

  void
  assert_wstr (const char *cmp, const wchar_t *exp, const wchar_t *real,
               const char *caller, int line)
  {
    if ((!exp ^ !real)
        || (exp
            && ((cmp[1] == '='
                 && ((cmp[0] == '=') ^ (wcscmp (exp, real) == 0)))
                || (cmp[1] == '~'
                    && ((cmp[0] == '=') ^ (wcsstr (exp, real) != NULL))))))
      {
        CTEST_ERR ("%s:%d  assertion failed, '%ls' %s '%ls'", caller, line,
                   exp, cmp, real);
      }
  }

  void
  assert_data (const unsigned char *exp, size_t expsize,
    ...
    for (i = 0; i < expsize; i++)
      {
        if (exp[i] != real[i])
          {
            CTEST_ERR ("%s:%d expected 0x%02x at offset %" PRIuMAX
                       " got 0x%02x",
  ...
  static bool
  get_compare_result (const char *cmp, int c3, bool eq)
  {
    if (cmp[0] == '<')
  ...
  void
  assert_compare (const char *cmp, intmax_t exp, intmax_t real,
                  const char *caller, int line)
  {
    ...
    if (!get_compare_result (cmp, c3, c3 == 0))
      {
        CTEST_ERR ("%s:%d  assertion failed, %" PRIdMAX " %s %" PRIdMAX "",
                   caller, line, exp, cmp, real);
      }
  }

  void
  assert_compare_u (const char *cmp, uintmax_t exp, uintmax_t real,
                    const char *caller, int line)
  {
    int c3 = (real < exp) - (exp < real);
    ...
    if (!get_compare_result (cmp, c3, c3 == 0))
      {
        CTEST_ERR ("%s:%d  assertion failed, %" PRIuMAX " %s %" PRIuMAX,
                   caller, line, exp, cmp, real);
      }
  }

  void
  assert_interval (intmax_t exp1, intmax_t exp2, intmax_t real,
                   const char *caller, int line)
  {
  ...
  static bool
  approximately_equal (double a, double b, double epsilon)
  {
    double d = a - b;
    if (d < 0)
      d = -d;
  ...
  void
  assert_dbl_compare (const char *cmp, double exp, double real, double tol,
                      const char *caller, int line)
  {
        ...
        if (filter (test))
          {
            ...
            printf ("TEST %d/%d %s:%s ", idx, total, test->ssname,
                    test->ttname);
            fflush (stdout);
            if (test->skip)
              {
