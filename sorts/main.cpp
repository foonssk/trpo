...
double
timer (int tip_sort, int a[], long long n)
{
  ...
  return chrono::duration_cast<chrono::microseconds> (end - start).count ()
         / 1e6; // Время в секундах
}

int
main ()
{
  setlocale (LC_ALL, "rus"); // Устанавливаем локаль для корректного
                             // отображения русских символов
  int m[100000]; // Объявляем массив для тестирования
  long long choice,
      len; // Переменные для выбора типа сортировки и длины массива
  ...
  uniform_int_distribution<> dis (
      0, 100000); // Установка диапазона случайных чисел

  while (true)
    { // Бесконечный цикл для повторения тестирования
